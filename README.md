...
7. Multi-sector cadence scheduler (penny fast, crypto medium, index slower):
   ...
   - Scheduler supports in-cycle AI decisions via direct provider APIs in `aiScheduler` with rule-engine fallback.
...
11. Launch control/API bridge (includes AI endpoints):
...
- `src/revenue_generator/ai_bridge.py` - direct provider signal generation + risk gate normalization
...
- `POST /api/ai/analyze` - run provider AI call and return risk-gated plan
...
- Set `ANTHROPIC_API_KEY` or `OPENAI_API_KEY` in `.env` so `/api/ai/analyze` can call a model provider.
